...
const fetchData = async ({url, limit}: fetchDataArguments): Promise<string[]> => {
    try {
        const response = await fetch(`${url}?limit=${limit}`);
        const data: PokemonApiResponse = await response.json();
        const getResult = data.results;
        const pokemonsList: Array<string> = getResult.map(
            (pokemon: pokemonURL) => {
...
export { fetchData, fetchEachPokemon, fetchSelectedPokemon, URL, LIMIT };